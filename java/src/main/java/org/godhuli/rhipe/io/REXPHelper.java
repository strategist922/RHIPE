...
import org.godhuli.rhipe.io.REXPProtos.REXP;
import org.godhuli.rhipe.io.REXPProtos.REXP.RClass;
...
public class REXPHelper  {
	...
	case STRING:
		...
		for(int i=0;i<length-1;i++){
		    org.godhuli.rhipe.io.REXPProtos.STRING si = r.getStringValue(i);
		    if( si.getIsNA() )
			sb.append("NA");
		    else{
		...
		org.godhuli.rhipe.io.REXPProtos.STRING si = r.getStringValue(length-1);
		if( si.getIsNA() )
		    sb.append("NA");
		else{
		    sb.append(squote);sb.append(si.getStrval());
	...
	case COMPLEX:
		...
		for(int i=0;i<length-1;i++){
		    org.godhuli.rhipe.io.REXPProtos.CMPLX ci = r.getComplexValue(i);
		    sb.append(ci.getReal());sb.append(ci.getImag());sb.append("i");
		    sb.append(fsep);
		}
		org.godhuli.rhipe.io.REXPProtos.CMPLX ci = r.getComplexValue(length-1);
